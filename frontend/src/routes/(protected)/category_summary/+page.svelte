<script lang="ts">
	...
	let sortedTags = $derived(
		[...(appData.data?.tags ?? [])].sort((a, b) => a.name.localeCompare(b.name))
	);

	let selectedTagIds = new SvelteSet<number>();
	let tagsInitialized = $state(false);

	// Initialize all tags as selected once data is loaded
	$effect(() => {
		if (appData.data && !tagsInitialized) {
			appData.data.tags.forEach((t) => selectedTagIds.add(t.id));
	...
	let monthsSpan = $derived.by(() => {
		...
		if (periodType === 'all' && transactions.length > 0) {
	...
	let treeData = $derived.by(() => {
		...
		const buildAndRollup = (parentId: number | null): TreeNode[] => {
			const childrenNodes = categories
...
{:else}
	...
	<div class="flex gap-2 mb-2">
		...
		<button
			onclick={collapseAll}
			class="text-sm text-blue-600 dark:text-blue-400 hover:underline">Collapse All</button
		>
	</div>
		...
		{#if showInExpenses || showInIncome || showInCombined}
			...
			>
				<div class="flex items-center" style="padding-left: {depth * 1.5}rem;">
					...
					{/if}
					<span class="font-medium {depth === 0 ? 'text-lg' : ''}"
	...
	<div class="bg-white dark:bg-gray-900 rounded shadow overflow-hidden">
		...
		{/each}
		{#if treeData.length === 0 || (appData.data?.transactions ?? []).length === 0}
