<script lang="ts">
    ...
    function handleEdit() {
        if (onEdit) onEdit();
        else goto(`/categories/${category.id}/edit?redirectTo=${encodeURIComponent(page.url.pathname)}`);
    }

    function handleDelete() {
        if (onDelete) onDelete();
        else goto(`/categories/${category.id}/delete?redirectTo=${encodeURIComponent(page.url.pathname)}`);
    }
</script>
