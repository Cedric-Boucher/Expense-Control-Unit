<script lang="ts">
    ...
    $: path = computePathParts(transaction.category, allCategories);
    ...
    function handleEdit() {
        if (onEdit) onEdit();
        else goto(`/transactions/${transaction.id}/edit?redirectTo=${encodeURIComponent(page.url.pathname)}`);
    }

    function handleDelete() {
        if (onDelete) onDelete();
        else goto(`/transactions/${transaction.id}/delete?redirectTo=${encodeURIComponent(page.url.pathname)}`);
    }
</script>
...
<li class="bg-white dark:bg-gray-800 shadow rounded p-4 flex justify-between items-start gap-4">
    ...
    {#if showActions}
        <div class="flex flex-col gap-2">
            <button type="button" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700" on:click={handleEdit}>Edit</button>
            <button type="button" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700" on:click={handleDelete}>Delete</button>
        </div>
    {/if}
</li>
